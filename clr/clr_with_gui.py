...
(Format: "A->Y1Y2..Yn" {Yi - single char} OR "A->" {epsilon})''', font='Helvetica -20', fg="black")
        ...
        self.cont = Button(
            frame2, fg="red", text="CONTINUE", command=self.start)
        self.cont.pack(ipadx=10, ipady=10, expand=1, side=BOTTOM)

        Button(bottomframe, text="QUIT", fg="red", command=master.destroy).pack(
    ...
    def start(self):
        ...
        for nt in ntl:
            ...
            self.text.insert(END, "\tFirst:\t{}\n".format(
                firstfollow.get_first(nt)))
            self.text.insert(END, "\tFollow:\t{}\n\n".format(
                firstfollow.get_follow(nt)))
